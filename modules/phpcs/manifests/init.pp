...
class phpcs (
	...
	if ( latest == $package ) {
		...
		exec { 'wordpress cs install':
			command => 'git clone -b master https://github.com/WordPress/WordPress-Coding-Standards /vagrant/extensions/phpcs/wpcs && phpcs --config-set installed_paths /vagrant/extensions/phpcs/wpcs',
			path    => [ '/bin/', '/sbin/' , '/usr/bin/', '/usr/sbin/' ],
